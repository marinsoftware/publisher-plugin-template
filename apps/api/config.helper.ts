...
let envFileLocation = path.join(__dirname, '../../conf/env.properties');
let yamlFileLocation = path.join(__dirname, '../../conf/config.yaml');
...
try {
  ...
  try {
    envFileLocation = path.join(__dirname, '../../../conf/env.properties');
    yamlFileLocation = path.join(__dirname, '../../../conf/config.yaml');
    propertiesFileContent = fs.readFileSync(envFileLocation, ENCODING.UTF_8);
  } catch (e){
...
export interface Config {
  ...
  LIMIT_BREACHED_EMAIL_TO: string;
  GELF: {
    host: string;
    port: number;
  };
  ...
  AUTH_TOKEN: string;
  VERSION: number;
  REDIRECT_URI_JUSTADS: string;
  REDIRECT_V1_URI_JUSTADS: string;
}
